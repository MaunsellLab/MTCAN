...
for file in fileList:
    ...
    if len(corrUnits) > 1:
        ...
        for comb in combs:
            ...
            for i in range(1, 5):
                ...
                # for j in [stimIndices[2], stimIndices[3]]:
            ...
            # print(f'session: {file}, n1: {n1}, n2: {n2}, offset: {i}')

            tempCorr = np.array(tempCorr)
            # print(tempCorr)
            # tempCorr = tempCorr / np.max(np.abs(tempCorr))
            # print('normalized corr')
            # print(tempCorr)
    ...
    for count in range(len(units)):
        ...
        if f'{seshDate}_{units[count]}' in unitList:
            spon = meanSpikeReshaped[count, 0, 0] / np.max(prefTransect)
            nullOnly = (np.concatenate(([meanSpikeReshaped[count, 1, 0]],
                                       meanSpikeReshaped[count, 0, 1::2]), axis=0) /
                        np.max(prefTransect))
            prefOnly = (np.concatenate(([meanSpikeReshaped[count, 2, 0]],
                                       meanSpikeReshaped[count, 0, 2::2]), axis=0) /
                        np.max(prefTransect))
            NP = meanSpikeReshaped[count, 1, 2::2] / np.max(prefTransect)
            PN = meanSpikeReshaped[count, 2, 1::2] / np.max(prefTransect)
            PP = meanSpikeReshaped[count, 2, 2::2] / np.max(prefTransect)
            NN = meanSpikeReshaped[count, 1, 1::2] / np.max(prefTransect)
            prefTransect = prefTransect / np.max(prefTransect)
...
off1PopMean = np.median(off1CorrPop)
off1SEM = np.std(off1CorrPop) / np.sqrt(len(off1CorrPop))
off2PopMean = np.median(off2CorrPop)
off2SEM = np.std(off2CorrPop) / np.sqrt(len(off2CorrPop))
off3PopMean = np.median(off3CorrPop)
off3SEM = np.std(off3CorrPop) / np.sqrt(len(off3CorrPop))
off4PopMean = np.median(off4CorrPop)
off4SEM = np.std(off4CorrPop) / np.sqrt(len(off4CorrPop))
...
fig, ax = plt.subplots()
ax.scatter(x, corrArrMean)
ax.plot(x, corrArrMean)
ax.violinplot(corrArr, x, widths=0.5, showmeans=True, showextrema=True, showmedians=True,
              bw_method=0.5)
ax.errorbar(x, corrArrMean, yerr=SEMArr, fmt='o', ecolor='black', color='black',
            markersize=2)
ax.set_ylabel('Spike Count Correlation (pop mean)')
ax.set_xlabel('Offset Position')
ax.set_title('Offset Position vs Rsc blocks (all blocks)')
plt.show()
...
fig.suptitle('correlations vs rf weight for all stim (single/paired)')
...
# ax1.set_title(f'pair rf weight vs rsc, corr= {stats.pearsonr(rfWeightPop, totCorrPop)[0]:.2f}')
ax1.invert_xaxis()
plt.show()
...
binMeanRFWeight = np.array([np.median(i) for i in equalBinsRFWeight])
binMeanNMIPop = np.array([np.median(i) for i in equalBinsNMIPop])
...
ax1.set_ylabel('NMI')
ax1.set_title(f'RF weight vs NMI, coef={stats.pearsonr(totRFWeightPop, totNMIPop)[0]:.2f}')
ax1.invert_xaxis()
...
for filler in range(1):
    ...
    for i in transectNormalized:
        ax1.plot(transX, i, color='black', alpha=0.1)
