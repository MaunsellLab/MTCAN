function [] = polarWithErrorBars(angle, avg, error)
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
n_data = length(angle);
fake = polarplot(angle, max(avg+error)*ones(1,n_data)); 
set(fake,'Visible','off'); 
hold on;
polarplot(angle, avg, '-sb', 'markerFaceColor', 'b');
for ni = 1 : n_data
  if (avg(ni)-error(ni)) < 0
    polarplot(angle(ni)*ones(1,3),[0, avg(ni), avg(ni)+error(ni)],'-b');
  else
    polarplot(angle(ni)*ones(1,3),[avg(ni)-error(ni), avg(ni), avg(ni)+error(ni)],'-b');
  end
end
hold off